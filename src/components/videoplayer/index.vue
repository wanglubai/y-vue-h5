<template>
  <div>
    ...
    <div class="video-mask" :class="{'maskShow':mask}"></div>
    <div class="video-mask2" :class="{'maskShow':mask}">
      <div class="mask-close" @click="closeMask"></div>
      <a id="linkBtn2" class="download2 report-rbi-click" data-label="h5-app-guozudw" data-cate="" data-action="click" href="javascript:;"></a>
      <div class="guozu-bg"></div>
    </div>
    <div class="jump_mask" v-show="jumpClose"></div>
    <div class="jump_dialog"  v-show="jumpClose">
      <div class="hd">
        <p>在<span class="c_orange">“龙珠直播”</span>中打开链接吗？</p>
      </div>
      <div class="fd">
        <a class="jump_btn" href="javascript:;" @click="closeJump">取消</a>
        <a id="moblink111" class="jump_btn jump_btn_open" @click="huanqiApp" href="javascript:;">打开</a>
      </div>
    </div>
    <div class="videoplayer">
      ...
      <div class="author-information clearfix">
        <div class="author-head">
          <img :src="roomstatus.logo" />
        </div>
        <div class="author-name-hot">
          <span>{{roomstatus.name}}</span>
          <div class="hotNum clearfix">
            <i></i>
            <span> {{roomstatus.onlineCount}}</span>
          </div>
        </div>
        <a id="linkBtn1" class=" report-rbi-click app-download" data-label="h5-dw-app" data-cate="" data-action="click" href="javascript:;"></a>
      </div>
    </div>
  </div>
</template>
    ...
    getData() {
      vueHttp.getLiveUrl({
        ...
        if(res.broadcast){
          ...
          this.matchId = res.broadcast.matchId;
          if(this.matchId > 0 && this.streamUri == ''){
            huanqiAppDownLink.setMoblink('moblink111');      //弹框
            this.jumpClose = true;
            return ;
          }
        }
